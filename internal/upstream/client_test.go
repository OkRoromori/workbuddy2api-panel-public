...
func TestFetchModelsOverlaysV3ConfigCapabilities(t *testing.T) {
	// CLI 目录给 flash 精简字段（128K / 固定 high）＋**只它下发的第三方模型**；
	// IDE /v3/config 给完整能力。两路合并口径：目录取并集，同名冲突时 IDE 条目优先。
	...
	c := testClient(func(r *http.Request) (*http.Response, error) {
		...
		case strings.HasSuffix(r.URL.Path, "/v3/config"):
			switch ua := r.Header.Get("User-Agent"); ua {
			case codeBuddyIDEUA:
				sawIDE = true
			...
			case codeBuddyCLIUA:
				sawCLI = true
				return jsonResp(200, `{"code":0,"data":{"models":[
					{"id":"deepseek-v4.1-flash","name":"Deepseek-V4.1-Flash","maxInputTokens":1000000,"maxOutputTokens":128000,"credits":"x0.03","supportsReasoning":true,"onlyReasoning":true,"reasoning":{"effort":"high","summary":"auto"}},
			...
			default:
				t.Errorf("v3/config UA=%q 既不是 IDE 也不是 CLI 路（两路必须都发）", ua)
				return jsonResp(200, `{"code":0,"data":{"models":[]}}`), nil
			}
		default:
	...
	if !sawIDE || !sawCLI {
		t.Fatalf("v3/config 两路都要探（IDE=%v CLI=%v）", sawIDE, sawCLI)
	}
	// 目录 = 并集：CLI 独有的模型必须进目录（这正是"国际服有 deepseek/gpt-6-astra 却不在
	// 列表里"那类漏列的修复点）。
	byID := map[string]ModelInfo{}
	for _, mi := range infos {
		byID[mi.ID] = mi
	...
	if _, ok := byID["gpt-6-astra"]; !ok {
		t.Fatalf("CLI 独有的 gpt-6-astra 未进目录: %+v", infos)
	}
	mi, ok := byID["deepseek-v4.1-flash"]
	if !ok {
		t.Fatalf("deepseek-v4.1-flash 缺失: %+v", infos)
	}
	// 同名冲突保留 IDE 条目：maxOut 393216（而非 CLI 的 128000）、档位含 low。
	if mi.MaxTokens != 393216 {
		t.Errorf("MaxTokens=%d want 393216（冲突时应保留 IDE 条目）", mi.MaxTokens)
	}
	...
	// 并集进来的条目要带上自己的能力（不能是空壳）。
	if ga := byID["gpt-6-astra"]; strings.Join(ga.Efforts, ",") != "low,medium,high,xhigh,max" || ga.Credits != "x6.67" {
		t.Errorf("gpt-6-astra 能力未透出: efforts=%v credits=%q", ga.Efforts, ga.Credits)
	}
}

// TestV3ConfigOneUAFailsStillUsesOther 两路独立容错：一路失败仍用另一路的目录
//（否则 CLI UA 侧偶发抖动会把整个模型目录打回企业端点，第三方模型族又整族消失）。
func TestV3ConfigOneUAFailsStillUsesOther(t *testing.T) {
	c := testClient(func(r *http.Request) (*http.Response, error) {
		switch {
		case strings.HasSuffix(r.URL.Path, "/v3/config"):
			...
			return jsonResp(200, `{"code":0,"data":{"models":[
				{"id":"cli-only-model","maxInputTokens":1000,"maxOutputTokens":8000}
			]}}`), nil
		default:
			return jsonResp(404, `{}`), nil
		}
	})
	byID, err := c.fetchV3ConfigModelMap(&auth.Auth{AccessToken: "at", UID: "u1", Domain: "copilot.tencent.com"})
	if err != nil {
		t.Fatalf("IDE 路失败不该让整体失败: %v", err)
	}
	if _, ok := byID["cli-only-model"]; !ok {
		t.Fatalf("应保留可用的 CLI 目录: %+v", byID)
	}
}
