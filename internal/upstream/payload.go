...
func PrepareBodyOptWithEffortsAndDefault(src []byte, sanitize bool, efforts map[string][]string, defaultEfforts map[string]string) []byte {
	...
	normalizeRoles(obj)
	// tool 配对三步（见 tool_pairing.go）：先合并、再重排、最后清孤儿。所有模型一律执行
	//（独立于 deepseek-only 的 sanitize 开关）。这是「让请求通过」的安全网——不完整配对
	// 的 tool_calls/tool 结果会让上游对之后每条消息都返 400，必须先行剔除；
	...
	// assistant.tool_calls」合成一条（Responses 协议并行调用的报文形状），是上游
