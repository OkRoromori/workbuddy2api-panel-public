	...
	c := &Client{
		HTTP: &http.Client{Transport: rtFunc(func(r *http.Request) (*http.Response, error) {
			...
			case strings.HasSuffix(r.URL.Path, "/v3/config"):
				// 模型目录探测固定走 IDE + CLI 两个 UA（两者目录互补，见
				// fetchV3ConfigModelMap）：既不接受配置 UA 覆盖，也不接受第三种 UA。
				got := r.Header.Get("User-Agent")
				if got != codeBuddyIDEUA && got != codeBuddyCLIUA {
					t.Errorf("v3/config UA = %q want %s 或 %s", got, codeBuddyIDEUA, codeBuddyCLIUA)
				}
