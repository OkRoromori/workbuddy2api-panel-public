...
// 数 MB 的合法 body 完整读入并原样打上游，网关既不 413 也不截断。
//
// 2026-09-20 线上事故：鉴权层（withAuth）以 1 MiB 为预检上限读取请求体，并把截断后的
// 结果回填 r.Body，>1 MiB 的长会话因此被砍成半截 JSON 打给上游 → 上游 400 code=11101
// （Unmarshal chat params failed: unexpected EOF）→ 客户端只看到 503 且每次重试稳定复现，
// 换号也无效。旧断言只查「网关不 413」，而 fake 上游照收截断体也返回 200，所以漏掉了这个
// 缺陷——故这里改为断言**上游实际收到**的 body 完整（pad 长度逐字节可验）。
func TestChatLargeBodyNoGatewayLimit(t *testing.T) {
	...
	if err := json.Unmarshal(gotBody, &sent); err != nil {
		t.Fatalf("上游收到的 body 不是合法 JSON（被截断？）: %v len=%d", err, len(gotBody))
	}
	if got, _ := sent["pad"].(string); len(got) != padLen {
		t.Errorf("上游收到 pad 长度=%d want %d（请求体被截断）", len(got), padLen)
	}
}

// TestChatBodyOverHardCapRejected 超过硬上限的请求体必须显式 413，而不是被静默截断后
// 打上游（截断即 half-JSON，上游只会回一个让人摸不着头脑的 11101）。
func TestChatBodyOverHardCapRejected(t *testing.T) {
	...
	if rec.Code != http.StatusRequestEntityTooLarge {
		t.Fatalf("code=%d want %d body=%s", rec.Code, http.StatusRequestEntityTooLarge, rec.Body)
	}
	if !strings.Contains(rec.Body.String(), "request_body_too_large") {
		t.Errorf("错误码不符: %s", rec.Body)
	}
}
...
func TestModelsEndpoint(t *testing.T) {
	...
	for _, m := range data {
		mm := m.(map[string]any)
		// 2026-09-20 起 id 一律裸名（域由 model_realm 配置解析，realm 字段单独透出）。
		if mm["id"] == "glm-5.2" {
			found = true
			if mm["realm"] != "cn" {
				t.Errorf("glm-5.2 realm=%v want cn", mm["realm"])
			}
		}
		if strings.Contains(mm["id"].(string), ":") {
			t.Errorf("id 不应带域前缀: %v", mm["id"])
		}
	}
	if !found {
		t.Error("glm-5.2 missing（裸名）")
	}
}
...
func TestModelsDynamic(t *testing.T) {
	...
	if !ids["dyn-model-a"] || !ids["glm-9.9"] {
		t.Errorf("dynamic ids missing（应为裸名）: %v", ids)
	}

	// 断言字段映射：maxInputTokens → context_length，maxOutputTokens → max_output_tokens
	// 注意字段名是 reasoning_supported_efforts / reasoning_default_effort（历史断言写的是
	// 无前缀名，且因 id 带 "cn:" 前缀而与 case 不匹配 → 那几条断言长期是死代码；裸名化
	// 之后才真正生效，此处一并订正）。
	for _, m := range data {
		mm := m.(map[string]any)
		switch mm["id"] {
...
func TestModelsNegativeCacheOnFetchFailure(t *testing.T) {
	...
	// 一轮探测 = 3 次上游调用（企业端点 + /v3/config 的 IDE UA 与 CLI UA 两路，三条全失败
	// 才进负缓存）。v3 两路是因为目录互补：只探 IDE UA 会漏掉第三方模型族。
	if got := calls.Load(); got != 3 {
		t.Errorf("want 3 probes (console + v3 x2 UA), got %d", got)
	}
	...
	if got := calls.Load(); got != 6 {
		t.Errorf("want 6 probes after cooldown (2 rounds x 3), got %d", got)
	}
}
...
// TestModelsListRespectsKeyPolicy /v1/models 必须按这把密钥的模型白名单过滤列表。
//
// 为什么：限制此前只体现在调用时的 403（model_not_allowed）——分享一把"只允许 5 个模型"
// 的密钥，对方客户端里仍列出全部模型（点哪个都报错），看起来像"指定模型没生效"。
// 列表与限制必须一致。白名单条目沿用历史形态（"cn:glm-5.3"）也要能命中（按裸名比对）。
func TestModelsListRespectsKeyPolicy(t *testing.T) {
	const key = "sk-policy-test"
	reset := func() {
		dynamicModelsCache.Lock()
		dynamicModelsCache.ids = nil
		dynamicModelsCache.fetched = time.Time{}
	...
	],"agents":[{"name":"cli","models":["glm-5.2","glm-5.3"]}]}}`
	up := newFakeUpstream(t, func(string) (int, string, bool) { return 200, catalog, false })
	p := testPoolWith(&auth.Auth{UID: "u1", AccessToken: "at", ExpiresAt: 9999999999})
	...
	cases := []struct {
		...
		{"无白名单 → 全部", nil, []string{"glm-5.2", "glm-5.3"}},
		{"白名单只允许一个", []string{"glm-5.2"}, []string{"glm-5.2"}},
		{"白名单沿用带前缀的历史条目", []string{"cn:glm-5.3"}, []string{"glm-5.3"}},
	}
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			reset()
			live := livecfg.New(livecfg.Snapshot{
				Keys:        map[string]string{key: "tester"},
				KeyPolicies: map[string]livecfg.KeyPolicy{httpauth.Fingerprint(key): {Models: c.models}},
			})
			h := NewHandler(Config{Pool: p, Upstream: up, Live: live})
			req := httptest.NewRequest("GET", "/v1/models", nil)
			...
			if strings.Join(got, ",") != strings.Join(c.want, ",") {
				t.Errorf("列表 = %v want %v（应按密钥白名单过滤）", got, c.want)
			}
		})
	}
}
