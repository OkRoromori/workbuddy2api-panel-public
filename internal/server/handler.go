...
type Config struct {
	...
	// Metrics 用量指标聚合器（可选；nil = 不统计）。每个 chat 请求出口喂一次，
	// 面板仪表盘读它的快照。nil 时完全不影响请求路径。
	Metrics *metrics.Registry

	// Audit 请求审计流水（可选；nil = 不审计）。每个 chat 请求出口投递一条明细，
	// 面板「使用日志」页读它。投递是非阻塞尽力而为的，不拖慢请求路径。
...
//
// Name 给人看（审计里显示"谁用的"），KeyID 给程序用（密钥页按它统计用量）。
// 两者都要：名字可改、指纹不可改，只存名字会让改名把历史用量丢掉。
type identity struct {
	Name   string
	KeyID  string
	Policy livecfg.KeyPolicy
}

// userCtxKey 请求上下文里携带 identity 的键。
type userCtxKey struct{}

// WithUser 把身份放进请求上下文。仅鉴权层与测试使用。
func WithUser(r *http.Request, name, keyID string) *http.Request {
	return r.WithContext(context.WithValue(r.Context(), userCtxKey{}, identity{Name: name, KeyID: keyID}))
}

func withIdentity(r *http.Request, id identity) *http.Request {
	return r.WithContext(context.WithValue(r.Context(), userCtxKey{}, id))
}

// identityOf 取回当前请求的身份；未经鉴权（或未启用鉴权）时为零值。
func identityOf(r *http.Request) identity {
	if v, ok := r.Context().Value(userCtxKey{}).(identity); ok {
		return v
	}
	return identity{}
}
...
// userOf 只取名字（旧调用点用）。
func userOf(r *http.Request) string { return identityOf(r).Name }
...
type Handler struct {
	...
	// wrap 可选的外层处理器（路径容错 / 临时诊断中间件）。
	wrap http.Handler

	// keyInFlight 子密钥在途计数（多密钥 MaxConcurrency 闸门）；
	// keySlotMu 保护它——请求并发进出，必须串行化。
	keySlotMu   sync.Mutex
...
func NewHandler(cfg Config) *Handler {
	...
	h := &Handler{cfg: cfg, mux: http.NewServeMux(), keyInFlight: map[string]int{}, wrap: nil}
	h.mux.HandleFunc("POST /v1/chat/completions", h.withAuth(h.chatCompletions))
	// Responses API（Codex 只会调这个，没有降级开关）：内部翻成 Chat 走同一条管线。
	h.mux.HandleFunc("POST /v1/responses", h.withAuth(h.responses))
	// Anthropic Messages API（Claude Code / ccswitch）：同样翻成 Chat。
	h.mux.HandleFunc("POST /v1/messages", h.withAuth(h.messages))
	h.mux.HandleFunc("GET /v1/models", h.withAuth(h.models))
	h.mux.HandleFunc("GET /status", h.withOwnerAuth(h.status))
	h.mux.HandleFunc("GET /healthz", h.healthz)
	if cfg.Panel != nil {
		h.mux.Handle("/panel/", cfg.Panel) // /panel → /panel/ 由 ServeMux 自动重定向
	}
	// 路径容错：客户端对 `base_url` 是否含 `/v1` 的做法彼此矛盾
	//（Codex 要求带、cc-switch 的 Fetch Models 会再拼一次），这里兜住。
	// 见 paths.go —— 只折叠已知端点，其余保留 404 语义。
	h.wrap = pathShim(h.mux)
	// 临时诊断：请求日志（仅 WB2A_LOG_REQ 打开时再包一层，见 diag.go）。
	if dir := os.Getenv(logReqEnv); dir != "" {
		h.wrap = logRequests(h.wrap, dir)
	}
	return h
}
...
// withAuth 校验 Bearer 密钥并把"是谁"注入请求上下文。
//
// 多密钥：命中哪把密钥决定审计流水里的用户（区别于累计指标——指标只知道总量，
// 分不出谁用的）。密钥表来自 livecfg 快照，面板里改了下一个请求即生效。
func (h *Handler) withAuth(next http.HandlerFunc) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		snapshot := h.loadLive()
		name, keyID, ok := httpauth.VerifyBearerAny(r, snapshot.ClientKeys())
		if !ok {
			writeOpenAIError(w, http.StatusUnauthorized, "invalid_api_key", "missing or invalid API key")
			return
		}
		policy := snapshot.KeyPolicies[keyID]
		if r.Method != http.MethodGet {
			...
			if len(raw) > maxRequestBodyBytes {
				writeOpenAIError(w, http.StatusRequestEntityTooLarge, "request_body_too_large",
					fmt.Sprintf("request body exceeds %d MiB", maxRequestBodyBytes>>20))
				return
			}
			...
			if !modelAllowed(policy.Models, peek.Model) {
				writeOpenAIError(w, http.StatusForbidden, "model_not_allowed",
					fmt.Sprintf("model %q is not allowed for this API key", peek.Model))
				return
			}
			// slotHeld 在**取号这一刻**定死，归还时用它而不是重置后的快照：
			// 面板把该密钥的上限热改成 0（不限并发）后，若归还时按 limit<=0 直接
			// return，名额会永久残留 → 上限调回 N 后该密钥所有请求 429，必须重启才恢复。
			slotHeld := policy.MaxConcurrency > 0 && keyID != ""
			if !h.acquireKeySlot(keyID, policy.MaxConcurrency) {
				writeOpenAIError(w, http.StatusTooManyRequests, "key_concurrency_limit",
					fmt.Sprintf("this API key allows at most %d concurrent requests", policy.MaxConcurrency))
				return
			}
			defer h.releaseKeySlot(keyID, slotHeld)
		}
		next(w, withIdentity(r, identity{Name: name, KeyID: keyID, Policy: policy}))
	}
}
...
// maxRequestBodyBytes 鉴权层读入请求体的硬上限（只为防 OOM），同时是「预检 + 原样回填」的
// 读取上限——两者必须同一个数。
//
// 鉴权层要先整个读出 body 才能取 model 字段做密钥白名单校验，读完**必须原样回填**。
// 2026-09-20 实机定位的事故：此前上限是 maxKeyPolicyPeek = 1 MiB，超出部分被静默砍掉后
// 仍照原样转发上游，上游对半截 JSON 返回 11101（Unmarshal chat params failed: unexpected
// EOF），客户端只看到 503 +「请重新发起对话」，长会话（请求体 >1 MiB）因此整条报废、
// 换号无效、每次重试稳定复现。结论：截断只能是显式 413，绝不静默。
//
// 64 MiB 的量级关系：实测最大的真实请求（ZCode 232 条消息的长会话）约 0.8 MiB，
// 1.4 MiB 已足以触发旧上限事故，故上限留足两个数量级余量。
const maxRequestBodyBytes = 64 << 20

// modelAllowed 密钥的模型白名单校验；空列表 = 全部放行。
//
// 域前缀只影响路由，不参与白名单语义：两侧都按**裸名**比对。这样白名单里历史遗留的
// "cn:glm-5.3-flash" 与客户端现在发的裸名 "glm-5.3-flash"（以及反过来的组合）都能命中——
// /v1/models 改发裸名（2026-09-20）之前建的密钥策略不会因此失效。
func modelAllowed(allowed []string, model string) bool {
	...
	for _, candidate := range allowed {
		...
		if _, cb := resolveModel(candidate); cb == bare {
			return true
		}
	}
	return false
...
// acquireKeySlot 子密钥并发闸门：超过 MaxConcurrency 时拒绝。
// limit<=0 表示不限并发，直接放行。
func (h *Handler) acquireKeySlot(keyID string, limit int) bool {
	if limit <= 0 || keyID == "" {
		return true
	}
	h.keySlotMu.Lock()
	defer h.keySlotMu.Unlock()
	if h.keyInFlight[keyID] >= limit {
		return false
	}
	h.keyInFlight[keyID]++
	return true
...
// releaseKeySlot 归还子密钥并发名额。held 由**取号时**的判定决定（见 withAuth），
// 不能用释放时读到的配置上限：面板把上限热改成 0 时，按当时的 limit<=0 return
// 会让名额永久残留。
func (h *Handler) releaseKeySlot(keyID string, held bool) {
	if !held || keyID == "" {
		return
	}
	h.keySlotMu.Lock()
	defer h.keySlotMu.Unlock()
	if h.keyInFlight[keyID] > 0 {
		h.keyInFlight[keyID]--
	}
}
...
func (h *Handler) models(w http.ResponseWriter, r *http.Request) {
	...
	// 按这把密钥的模型白名单过滤列表。
	//
	// 为什么必须过滤：限制只体现在调用时的 403（model_not_allowed）——客户端拉到的仍是**全部**
	// 模型，于是"分享给别人一把只允许 5 个模型的密钥"，对方客户端里却列出几十个模型（点哪个都
	// 报错），看起来像"指定模型没生效"。这里让列表与限制一致：连看都看不到，就不会误点。
	// 注意只影响**列表展示**：调用侧的白名单判定（modelAllowed）仍是唯一权威，不依赖此过滤。
	if policy := identityOf(r).Policy; len(policy.Models) > 0 {
		filtered := make([]map[string]any, 0, len(list))
		for _, entry := range list {
			id, _ := entry["id"].(string)
			if modelAllowed(policy.Models, id) {
...
// modelList 模型列表（OpenAI /v1/models 口径）。
//
// id 一律是**裸模型名**（2026-09-20 变更）：此前 CN 模型加 "cn:"、global 加 "global:"，
// 客户端拉完列表还得手工删前缀才能填进配置；而域只是网关的路由细节。现在归属由网关按
// model_realm 配置解析（见 resolveModelWithRealm），客户端想强制某域时仍可写
// "global:xxx"。realm 字段仍透出（非标准但无害），供客户端展示徽标。
//
// 重名模型（两域都有同名，如 glm-5.3）只出一条，取配置选中的那一侧的元数据——
// 「列表显示的是哪一域」与「实际调用落在哪一域」共用同一份解析，不会漂移。
// 纯动态：动态拉取失败/无号 → 该域空列表，无静态兜底。
func (h *Handler) modelList() []map[string]any {
	cnEntries := map[string]map[string]any{}
	globalEntries := map[string]map[string]any{}
	var cnOrder, globalOrder []string
	for _, mi := range h.fetchDynamicModels() {
		...
		cnEntries[mi.ID] = entry
		cnOrder = append(cnOrder, mi.ID)
	}
	...
	if h.cfg.GlobalEnabled {
		...
		for _, id := range globalIDs {
			...
			globalEntries[id] = entry
			globalOrder = append(globalOrder, id)
		}
	}

	// 组装：CN 顺序为主，global 独有的追加在后。重名模型（两域同名）只出一条，取
	// model_realm 选中的那一侧元数据——与请求侧共用 resolveModelWithRealm 同一份解析，
	// 「列表显示哪一域」与「实际调用落哪一域」不会漂移。
	snap := h.loadLive()
	return assembleModelList(cnEntries, globalEntries, cnOrder, globalOrder, snap.ModelRealmPrefer, snap.ModelRealmPins)
}

// assembleModelList 双域条目按**裸名**合并成一份 /v1/models 列表。
//
//   - CN 顺序为主（保持上游返回顺序），global 独有的追加在后；
//   - 重名模型只出一条：取 model_realm 选中那一侧的条目（含该域自己的 context_length /
//     倍率 / effort 档位等元数据，不混合两域字段）；
//   - 域解析与请求侧共用 resolveModelWithRealm，因此列表里显示归属哪个域，调用时就落哪个域。
//
// 抽成纯函数是为了可测：双域列表要构造两套假上游，成本高且脆；这里用合成条目直接钉住
// 「去重 + 优先级 + 顺序」三件事。
func assembleModelList(cnEntries, globalEntries map[string]map[string]any, cnOrder, globalOrder []string, prefer string, pins map[string]string) []map[string]any {
	out := make([]map[string]any, 0, len(cnOrder)+len(globalOrder))
	for _, id := range cnOrder {
		if g, dup := globalEntries[id]; dup {
			if realm, _ := resolveModelWithRealm(id, prefer, pins); realm == "global" {
				out = append(out, g)
				continue
			}
		}
		if e, ok := cnEntries[id]; ok {
			out = append(out, e)
		}
	}
	for _, id := range globalOrder {
		if _, dup := cnEntries[id]; dup {
			continue // 重名：上面已按优先级出过一条
		}
		if e, ok := globalEntries[id]; ok {
			out = append(out, e)
		}
	}
	return out
...
// modelRealmCatalog 装配请求侧「域校准」所需的目录视图（见 resolveModelRealm）。
//
// 两侧都**只读缓存、不触发探测**：聊天请求不该为了解析域去拉一次目录。
// 目录**不要求新鲜**（不查 TTL）：它只用来回答"哪个域有这个模型"，稍旧的目录比"没有目录"
// 有用得多——否则缓存一过期，单域模型又会被按偏好送错域并获 11102（实测踩到过）。
// 未缓存的一侧 CNKnown/GlobalKnown=false，解析退回配置偏好（不猜）。
func (h *Handler) modelRealmCatalog() ModelRealmCatalog {
	cat := ModelRealmCatalog{}
	dynamicModelsCache.RLock()
	if len(dynamicModelsCache.ids) > 0 {
		cat.CNKnown = true
		cat.CN = make([]string, 0, len(dynamicModelsCache.ids))
		for _, mi := range dynamicModelsCache.ids {
			cat.CN = append(cat.CN, mi.ID)
		}
	}
	dynamicModelsCache.RUnlock()
	if h.cfg.Upstream != nil {
		if names := h.cfg.Upstream.GlobalModelNamesCached(); len(names) > 0 {
			cat.GlobalKnown = true
			cat.Global = names
		}
	}
	return cat
...
// WarmModelRealmCatalog 预热域校准目录（启动后调一次，后台执行）。
//
// 为什么需要：目录缓存在进程内，重启后为空；冷目录期间的请求会退回配置偏好，只有 global 有的
// 模型（gpt-6-astra 等）会被送进国服并获 11102——实测部署后头几个请求就是这么挂的。
// 预热把这段窗口压到最短，代价是启动时各拉一次目录（与 /v1/models 共用同一份缓存）。
func (h *Handler) WarmModelRealmCatalog() {
	_ = h.fetchDynamicModels()
	h.fetchGlobalModels()
}

// fetchGlobalModels 返回 global 模型名单（纯动态探测结果）及被探测账号。// 缓存/失败回落封在 upstream.FetchGlobalModels（内部 1h + 5min 负缓存）。
...
func (h *Handler) chatCompletions(w http.ResponseWriter, r *http.Request) {
	...
	// 域解析：model 名可能带 "[realm:]" 前缀（客户端强制指定域），也可为裸名（常态，
	// /v1/models 下发的就是裸名）。剥出 realm + bareModel，bareModel 用于选号/粘性/出站
	// body 重写（前缀只是网关侧路由协议，上游只认裸名）。
	//
	// 解析顺序：**目录事实**（只有单一域有的模型强制那一域）→ 逐模型 pin → 全局 prefer。
	...
	for i := 0; i < h.cfg.MaxRotate; i++ {
		...
		if acct == nil {
			...
			// 软解析（按 prefer 选出来的域）在该域**没有可用账号**时回落另一域：
			// 用户把默认域设成国际服、国际服号却全在冷却/占满，请求不该就此失败。
			// 目录证明只有单域有、或用户显式钉死/写前缀的情况不会走到这里（softRealm=false）——
			// 那是明确意图，宁可报错也不悄悄换域。
			if acct == nil && softRealm {
				other := otherRealm(realm)
				if a2 := h.cfg.Pool.PickExcludingForRealm(tried, bareModel, other); a2 != nil {
					log.Printf("realm fallback model=%s %s → %s（%s 无可用账号）", bareModel, realm, other, realm)
					realm = other // 后续（粘性/统计/日志）都按实际落地的域
					acct = a2
				}
			}
		}
		...
		// 分域统计（第 2 步）：按**账号实际所属域**记账（不是解析出的期望域）——跨域回落时
		// 记的必须是真正服务的那个域，否则域视图的数字会张冠李戴。
		st.realm = acct.Realm()
		tried[acct.UID] = true
...
// isUpstreamTimeout 判断这一跳的失败是否属于「上游超时 / 停滞」。
//
//	timeout 类：net.Error.Timeout()（ResponseHeaderTimeout / Client.Timeout 等）、
//	           context.DeadlineExceeded / os.ErrDeadlineExceeded；
//	stalled：  客户端仍在（clientGone=false）却拿到 context.Canceled——只可能是本进程
//	           的空闲看门狗（monitorBody）掐的流，也就是上游不再吐数据。
//
// 客户端主动断连（clientGone=true）**不算**超时：那条路径照旧只换号不罚号，
// 语义是"人走了"，不是"上游慢"。超时的正确处置是**不换号**——换号撞的是同一个
// 慢上游，只会把客户端拖到 MaxRotate × header_timeout。
