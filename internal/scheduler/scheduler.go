...
type Config struct {
	...
	// Metrics 用量指标聚合器（可选；nil = 不统计）。余额刷新时回喂积分采样，
	// 面板仪表盘的「积分余额走势」「积分续航」读它（本地功能线）。
...
type Scheduler struct {
	...
	// runMu/running 保证同类批量任务同时只跑一轮（beginRun/endRun）。
	runMu   sync.Mutex
	running map[string]bool

	// 验活（probe.go）：间隔与模型/提示词都可热改。
	// probeCfg 用 RWMutex 而不是 atomic：它是两个字符串，凑一个不可变结构体再换指针
	// 也可以，但读频率极低（每轮每账号读一次），锁的开销完全不值得优化。
	probeMu       sync.RWMutex
	probeCfg      probeConfig
	probeInterval atomic.Int64
	// rearmProbe 与 rearmSchedule/rearmBalance 同理：间隔热改时唤醒循环重排。
	// 独立 channel 是必须的——同一个 channel 被多个 select 消费会丢信号。
	rearmProbe chan struct{}
}

// probeConfig 验活的可热改参数。
...
func New(cfg Config) *Scheduler {
	...
	return &Scheduler{
		...
		rearmProbe:    make(chan struct{}, 1),
	}
}

// beginRun/endRun 保证同类批量任务同时只跑一轮。面板重复点击或恰逢定时触发时，
// 后来的同类任务直接跳过，避免重复签到、刷新以及验活失败计数被并发放大。
func (s *Scheduler) beginRun(kind string) bool {
	s.runMu.Lock()
	...
	if s.running[kind] {
		log.Printf("scheduler: %s 已在执行，跳过重复触发", kind)
		return false
	}
	s.running[kind] = true
	return true
...
func (s *Scheduler) endRun(kind string) {
	s.runMu.Lock()
	delete(s.running, kind)
	s.runMu.Unlock()
}

// batchDoneTag 完成日志的固定前缀（与 panel handler 的「panel: 手动全量 X 已触发」同源，
// 便于人眼/grep 配对：搜「手动全量」配对找本轮触发点，搜 batchDoneTag 配对找收尾点）。
const batchDoneTag = "[" + "batch" + "]"

// logBatchDone 批量任务收尾汇总行（签到/旅行/活跃/保活/验活共用同一格式）。
func logBatchDone(label string, total, processed, failed int, start time.Time) {
	dur := time.Since(start).Round(time.Millisecond)
	skipped := total - processed
	if skipped < 0 {
		skipped = 0 // 防御：调用方口径不一致时也别打出负数
	}
	switch {
	case processed == 0 && failed == 0:
		// 池空 / 全员被跳过（如 disabled、无 token）。仍打一条，
		// 否则面板「已触发」之后什么都没动静，用户不知道是没跑还是跑完了。
		log.Printf("\a%s 全量 %s 完成：池内 %d 个账号，本轮均跳过（耗时 %s）",
			batchDoneTag, label, total, dur)
	case failed == 0:
		log.Printf("\a%s 全量 %s 完成 ✅ 共 %d 个账号（%d 跳过）耗时 %s",
			batchDoneTag, label, total, skipped, dur)
	default:
		log.Printf("\a%s 全量 %s 完成 ✅ 共 %d 个账号（%d 跳过 / %d 失败）耗时 %s — 查看失败账号 grep 上述输出",
			batchDoneTag, label, total, skipped, failed, dur)
	}
}
...
//
// 本地功能线增强：除池内 credits 外，还回喂指标聚合器（面板仪表盘的
// 「积分余额走势」与「积分续航」读它）——池级合计与到期批次在批末一次性提交。
func (s *Scheduler) RunBalanceRefreshNow() {
	...
	var wg sync.WaitGroup
	// 池级积分合计：NoteCredits 是逐账号并发写入的，池级数字要等全批到齐才知道，
	// 所以在这里单独汇总，wg.Wait() 后一次性交给指标聚合器。
	var sumMu sync.Mutex
	var remainSum, usedSum int64
	// 域级合计（第 3 步）：同一批数据按账号所属域再累一份，池级那份一行不动。
	realmAgg := map[string]*realmCreditAgg{}
	realmExp := map[string]map[string]*upstream.ExpiryBatch{}
	// 到期批次要跨 goroutine 累加，单独一把锁 + map。
	var expMu sync.Mutex
	expByDay := map[string]*upstream.ExpiryBatch{}
	for _, st := range s.cfg.Pool.List() {
		total++
		if st.Disabled {
			continue
		...
		go func(a *auth.Auth, uid string) {
			defer wg.Done()
			remain, used, exp, err := s.cfg.Upstream.UserResourcePackages(a)
			if err != nil {
			...
			s.feedCredits(uid, a.Realm(), used)
			// 快过期子集：窗口内的到期批次余额合计（pool 据此优先消耗）。
			soon := int64(0)
			if s.cfg.ExpiringSoonWindow > 0 {
				cut := time.Now().Add(s.cfg.ExpiringSoonWindow)
				for _, b := range exp {
					if !b.At.After(cut) {
						soon += b.Remain
					}
			...
			remainSum += remain
			usedSum += used
			// 域级合计（第 3 步）：账号只属于一个域，累进对应域即可，池级数字不受影响。
			if r := a.Realm(); r == "cn" || r == "global" {
				agg := realmAgg[r]
				if agg == nil {
					agg = &realmCreditAgg{}
					realmAgg[r] = agg
			...
			sumMu.Unlock()
			// 把该账号的到期批次累进池级 map。同一失效日的多个包/多个账号要相加，
			// 否则前端看到的「那天会失效多少」会偏小。
			expMu.Lock()
			for _, b := range exp {
				key := b.At.Format("2006-01-02")
				if cur, ok := expByDay[key]; ok {
					cur.Remain += b.Remain
				} else {
					bb := b
					expByDay[key] = &bb
			...
			// 域级到期批次：面板切到某域时，「那天会失效多少」也该只看该域。
			if r := a.Realm(); r == "cn" || r == "global" {
				m := realmExp[r]
				if m == nil {
					m = map[string]*upstream.ExpiryBatch{}
					realmExp[r] = m
				}
				for _, b := range exp {
					key := b.At.Format("2006-01-02")
			...
			expMu.Unlock()
			if soon > 0 {
				s.cfg.Pool.SetCreditsDetailed(uid, remain, remain+used, soon)
			} else {
				s.cfg.Pool.ReenableIfCredits(uid, remain, remain+used)
			}
		}(a, st.UID)
	}
	wg.Wait()
	// 各账号的「会过期的积分」批次：跨账号按失效日合并后再交给指标聚合器。
	expMu.Lock()
	expiring := mergeExpiry(expByDay)
	// 域级到期批次也合并好（面板切到某域时只看该域的到期分布）。
	for realm, m := range realmExp {
		if agg := realmAgg[realm]; agg != nil {
			agg.expiring = mergeExpiry(m)
		}
	}
	expMu.Unlock()
	if s.cfg.Metrics != nil {
		s.cfg.Metrics.SetCreditTotals("", remainSum, usedSum, time.Now(), expiring)
		// 域级积分口径（第 3 步）：同一批数据按域再写一份，供面板的国服/国际服视图切换。
		for realm, agg := range realmAgg {
			s.cfg.Metrics.SetCreditTotals(realm, agg.remain, agg.used, time.Now(), agg.expiring)
		}
	}
	logBatchDone("余额刷新", total, processed, failed, start)
}

// feedCredits 把单账号的本周期已用积分喂给指标聚合器（算积分→token 实测比率用）。
// realm 为账号所属域：域级比率只在同域账号之间差分（跨域混算会得出没有意义的"平均倍率"）。
func (s *Scheduler) feedCredits(uid, realm string, used int64) {
	if s.cfg.Metrics != nil {
		s.cfg.Metrics.NoteCredits(realm, uid, used)
	}
}

// realmCreditAgg 一个域的余额合计与到期批次（一轮刷新内累加，wg.Wait 后一次写库）。
type realmCreditAgg struct {
	remain   int64
	used     int64
	expiring []metrics.ExpiryBatch
}

// mergeExpiry 跨账号按失效日合并「会过期的积分」（按日期升序返回）。
func mergeExpiry(byDay map[string]*upstream.ExpiryBatch) []metrics.ExpiryBatch {
	out := make([]metrics.ExpiryBatch, 0, len(byDay))
	for _, b := range byDay {
		out = append(out, metrics.ExpiryBatch{At: b.At, Remain: b.Remain})
	}
