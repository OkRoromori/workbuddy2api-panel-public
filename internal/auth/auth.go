...
// HasAccessToken 报告是否持有 AccessToken（调度器跳过无凭证账号用）。
func (a *Auth) HasAccessToken() bool {
	return strings.TrimSpace(a.AccessTokenValue()) != ""
}

// HasRefreshToken 报告是否持有 RefreshToken（死号自检跳过无法证明存活的号用）。
func (a *Auth) HasRefreshToken() bool {
	return strings.TrimSpace(a.RefreshTokenValue()) != ""
}
