...
// mergeMPPending 合并小程序口径待办——**扫描与执行队列必须共用这一份口径**。
//
// 为什么要合并：school_season（校园日）与 Sequential_Tasks_1（小程序首对话）只在 mp 头
// 列表下发，默认口径的 ListTasks 里没有；mp 列表是默认口径的超集，按 task_code 去重即可。
// mp 列表失败静默——没有 mp 任务的部署、活动结束时零影响。
//
// 为什么必须共用：此前扫描合并了、执行队列漏合并，于是"扫描显示有待办，点执行全部待办
// 却报无可执行"。两处只要有一处漏改就会重现，故收成一个函数。
func (p *Panel) mergeMPPending(a *auth.Auth, grow []upstream.Task) []upstream.Task {
	mpTasks, err := p.cfg.Upstream.ListTasksMP(a)
	if err != nil {
		return grow
	}
	seen := make(map[string]bool, len(grow))
	for _, t := range grow {
...
func (p *Panel) tasksScanAll(w http.ResponseWriter, r *http.Request) {
	...
	for i, st := range states {
		...
		go func(i int, uid string) {
			...
			// 小程序口径待办合并（口径与去重见 mergeMPPending）。
			it.Growth = p.mergeMPPending(a, it.Growth)
		...
		go func(a *auth.Auth, wantSchool bool) {
			...
			if body.Growth {
				if tasks, err := p.cfg.Upstream.ListTasks(a); err == nil {
					...
					// 同一口径合并（此前漏了这一步：扫描显示待办、队列报无可执行）。
					one.grow = p.mergeMPPending(a, one.grow)
