...
| 🆕 | **Server-side half of key sharing** | The friend script generated by `internal/panel/tunnel_share.go` **embeds an Ed25519 private key** and pairs with a restricted server account (`wb2tunnel`, allowed to forward `127.0.0.1:7863` only — no command execution, no TTY). Every click of "Share" rotates the key, and deleting a key revokes the tunnel authorisation |
...
| Keys 🆕 | Key list with usage, rename, model whitelist (searchable and filterable by realm), per-key concurrency limit, generation of a dedicated connection script for a friend |
